...
//...........................................................................
    static final String NAME = "TwofishAlgorithm";
...
//...........................................................................
    static {
        ...
        if (DEBUG && debuglevel > 8) {
            ...
            for (i = 0; i < 64; i++) {
                for (j = 0; j < 4; j++) System.out.print("0x" + intToString(MDS[0][i * 4 + j]) + ", ");
                System.out.println();
            }
            System.out.println();
            System.out.println("MDS[1][]:");
            for (i = 0; i < 64; i++) {
                for (j = 0; j < 4; j++) System.out.print("0x" + intToString(MDS[1][i * 4 + j]) + ", ");
                System.out.println();
            }
            System.out.println();
            System.out.println("MDS[2][]:");
            for (i = 0; i < 64; i++) {
                for (j = 0; j < 4; j++) System.out.print("0x" + intToString(MDS[2][i * 4 + j]) + ", ");
                System.out.println();
            }
            System.out.println();
            System.out.println("MDS[3][]:");
            for (i = 0; i < 64; i++) {
                for (j = 0; j < 4; j++) System.out.print("0x" + intToString(MDS[3][i * 4 + j]) + ", ");
                System.out.println();
            }
            System.out.println();
...
//...........................................................................
    ...
    private static synchronized Object makeKey(byte[] k)
        ...
        if (DEBUG && debuglevel > 7) {
            ...
            for (i = 0; i < 64; i++) {
                for (j = 0; j < 4; j++) System.out.print("0x" + intToString(sBox[i * 4 + j]) + ", ");
                System.out.println();
            }
            System.out.println();
            for (i = 0; i < 64; i++) {
                for (j = 0; j < 4; j++) System.out.print("0x" + intToString(sBox[256 + i * 4 + j]) + ", ");
                System.out.println();
            }
            System.out.println();
            for (i = 0; i < 64; i++) {
                for (j = 0; j < 4; j++) System.out.print("0x" + intToString(sBox[512 + i * 4 + j]) + ", ");
                System.out.println();
            }
            System.out.println();
            for (i = 0; i < 64; i++) {
                for (j = 0; j < 4; j++) System.out.print("0x" + intToString(sBox[768 + i * 4 + j]) + ", ");
                System.out.println();
            }
            System.out.println();
...
//...........................................................................
    ...
    private static byte[] flatten2DArray(byte[][] array) {
    ...
    public static String textEncrypt(String plainText, String sourceKey, Integer keySize) throws Exception {
        byte[][] plainTextChunks = divideArray(plainText.getBytes(), BLOCK_SIZE);
        byte[] sourceKeyBytes = sourceKey.getBytes(StandardCharsets.UTF_8);
        sourceKeyBytes = divideArray(sourceKeyBytes, keySize)[0];
        byte[] encodedBytes;
        Object sessionKey = makeKey(sourceKeyBytes);
        byte[][] encodedChunks = new byte[plainTextChunks.length][BLOCK_SIZE];
        int chunkCount = 0;
        for (byte[] plainTextChunk : plainTextChunks) {
            encodedChunks[chunkCount++] = blockEncrypt(plainTextChunk, 0, sessionKey);
            System.out.println(toString(blockEncrypt(plainTextChunk, 0, sessionKey)));
        }
        encodedBytes = flatten2DArray(encodedChunks);
        return toString(encodedBytes);
    }

    public static String textDecrypt(String encodedText, String sourceKey, Integer keySize) throws Exception {
        byte[] encodedBytes = hexStringToByteArray(encodedText);
        byte[][] encodedTextChunks = divideArray(encodedBytes, BLOCK_SIZE);
        System.out.println(encodedBytes.length + " " + encodedTextChunks.length);
        byte[] sourceKeyBytes = sourceKey.getBytes(StandardCharsets.UTF_8);
        sourceKeyBytes = divideArray(sourceKeyBytes, keySize)[0];
        String decodedText = "";
        Object sessionKey = makeKey(sourceKeyBytes);
        byte[][] decodedChunks = new byte[encodedTextChunks.length][BLOCK_SIZE];
        int chunkCount = 0;
        for (byte[] encodedTextChunk : encodedTextChunks) {
            System.out.println(toString(encodedTextChunk));
            decodedChunks[chunkCount++] = blockDecrypt(encodedTextChunk, 0, sessionKey);
            System.out.println(new String(blockDecrypt(encodedTextChunk, 0, sessionKey), StandardCharsets.UTF_8));
        }
        byte[] decodedBytes = flatten2DArray(decodedChunks);
        decodedText = new String(decodedBytes, StandardCharsets.UTF_8).trim();
        return decodedText;
    }
